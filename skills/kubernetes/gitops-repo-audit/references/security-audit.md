...
- [ ] **No cross-namespace source refs**: In multi-tenant setups, check for `sourceRef.namespace` where a Kustomization or HelmRelease in one namespace references a source in another namespace. This bypasses tenant isolation. Enable `--no-cross-namespace-refs=true` on controllers (via FluxInstance `multitenant: true` or kustomize patches). Always report cross-namespace refs as a finding. Severity: Critical when the repo hosts multiple tenants or the FluxInstance sets `multitenant: true`; Warning otherwise (recommend co-locating the source with its consumer or enabling multi-tenancy lockdown). The only case to downgrade to Info is a platform-team pipeline where the FluxInstance explicitly declares `cluster.multitenant: false` *and* the referencing Kustomizations are generated by a ResourceSet from `flux-system` (directory-driven monorepo) — do not infer a "single trust domain" from a bootstrap repo or from hand-written HelmReleases/Kustomizations
